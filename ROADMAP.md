...
| A2 | Derive suitable POSIX signatures from the agent database. | After A1. Verify executable boundaries, Windows-only aliases and misleading argument matches; preserve `opencode` / `grok` coverage. The database's 112 agents / 265 names are not a count of detectable Linux agents or processes. Derive the usable subset. |
