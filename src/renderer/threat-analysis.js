...
<div class="section"><div class="section-title">THREAT LEVEL</div><span class="rating">${escapeHtml(analysis.riskRating || 'UNKNOWN')}</span></div>
<div class="section"><div class="section-title">EXECUTIVE SUMMARY</div><p>${escapeHtml(analysis.summary || '').replace(/\n/g, '<br>')}</p></div>
${analysis.findings && analysis.findings.length > 0 ? `<div class="section"><div class="section-title">FINDINGS</div><ul>${analysis.findings.map(f => `<li>${escapeHtml(f)}</li>`).join('')}</ul></div>` : ''}
${analysis.recommendations && analysis.recommendations.length > 0 ? `<div class="section"><div class="section-title">RECOMMENDED ACTIONS</div><ul>${analysis.recommendations.map(r => `<li>${escapeHtml(r)}</li>`).join('')}</ul></div>` : ''}
