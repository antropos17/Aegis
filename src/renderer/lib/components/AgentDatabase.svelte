...
<div class="db-wrap">
  <input
    class="db-search"
    type="text"
    placeholder={$t('rules.database.search_placeholder')}
    bind:value={search}
  />
    ...
    <table class="db-table">
      <thead>
        <tr>
          ...
          {/each}
          {#if onedit || ondelete}<th class="th-act">{$t('rules.database.actions')}</th>{/if}
        </tr>
      </thead>
      <tbody>
        ...
        {:else}
          <tr><td colspan="5" class="td-empty">{$t('rules.database.no_agents')}</td></tr>
        {/each}
      </tbody>
    </table>
  </div>
  ...
  <span class="db-count"
    >{$t('rules.database.count', { filtered: filtered.length, total: agents.length })}</span
  >
</div>
