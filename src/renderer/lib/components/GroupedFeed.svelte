<script>
  ...
  let unified = $derived.by(() => {
    ...
    const netEvs = $network.map(c => ({
      agent: c.agent || 'Unknown', timestamp: c.timestamp || Date.now(),
      file: `${c.domain || c.remoteIp || '?'}:${c.remotePort || '?'}`,
      action: c.state || 'established', sensitive: !!c.flagged,
      reason: c.flagged ? 'Unknown domain' : '', flagged: !!c.flagged,
      _type: 'network', _domain: c.domain || '', _ip: c.remoteIp || '',
    }));
  ...
  let groups = $derived.by(() => {
    ...
    return [...map.entries()].map(([name, evs]) => {
      ...
      return {
        name, count: evs.length, lastActivity: evs[0]?.timestamp || 0,
        severity: maxSev(evs), riskScore: Math.round(a?.riskScore || 0),
    ...
    {#each groups as group (group.name)}
      ...
      {#if open}
        <div class="group-body">
          {#each [
            { label: 'File Access', evs: group.fileEvents },
            { label: 'Config Access', evs: group.configEvents },
            { label: 'Network', evs: group.networkEvents },
          ] as sub (sub.label)}
            {#if sub.evs.length > 0}
...
<style>
  ...
  .feed-empty { padding: 40px 20px; text-align: center; font: var(--md-sys-typescale-body-medium); color: var(--md-sys-color-on-surface-variant); }

  .group-header {
    display: flex; align-items: center; gap: 8px; width: 100%; padding: 8px 12px;
    background: var(--md-sys-color-surface-container-low); border: none;
    border-bottom: 1px solid rgba(255, 255, 255, 0.04); cursor: pointer;
    font-size: 12px; color: var(--md-sys-color-on-surface); text-align: left;
    transition: background 0.15s ease;
  }
  .group-header:hover { background: var(--md-sys-color-surface-container); }
  .chevron { font-size: 10px; width: 12px; color: var(--md-sys-color-on-surface-variant); }
  .group-name { font-weight: 700; flex-shrink: 0; }
  .group-count { font-family: 'DM Mono', monospace; color: var(--md-sys-color-on-surface-variant); font-size: 10px; }
  .group-time { font-family: 'DM Mono', monospace; color: var(--md-sys-color-on-surface-variant); font-size: 10px; margin-left: auto; }

  .group-badge {
    font-size: 9px; font-weight: 700; padding: 1px 8px;
    border-radius: var(--md-sys-shape-corner-full); flex-shrink: 0;
    background: rgba(122, 138, 158, 0.1); color: var(--md-sys-color-on-surface-variant);
  }
  .group-badge.sev-critical { background: rgba(200, 90, 90, 0.15); color: var(--md-sys-color-error); }
  .group-badge.sev-high { background: rgba(200, 168, 78, 0.12); color: var(--md-sys-color-secondary); }
  .group-badge.sev-medium { background: rgba(66, 153, 225, 0.12); color: var(--md-sys-color-primary); }

  .group-body { padding-left: 12px; border-left: 2px solid rgba(255, 255, 255, 0.04); margin-left: 6px; }
  .sub-label {
    font-size: 9px; font-weight: 700; letter-spacing: 0.5px;
    color: var(--md-sys-color-on-surface-variant); padding: 6px 12px 2px; text-transform: uppercase;
  }
  ...
  .event-row {
    ...
    color: var(--md-sys-color-on-surface); text-align: left; transition: background 0.15s ease;
  }
  .event-row.odd { background: rgba(255, 255, 255, 0.01); }
  ...
  .badge-config { background: rgba(200, 168, 78, 0.12); color: var(--md-sys-color-secondary); }
  .feed-repeat { font-size: 9px; font-weight: 700; font-family: 'DM Mono', monospace; color: var(--md-sys-color-on-surface-variant); opacity: 0.7; flex-shrink: 0; }
