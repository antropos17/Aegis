...
/** Compute the existing exposure model with canonical instance joins.
 * @param $agents Live observations
 * @param $events Retained file events
 * @param $anomaliesByInstance Backend anomaly levels
 * @param $network Network observations
 * @param $fp Saved false positives
 * @returns Enriched instance records
 * @since 0.14.1
 */
export function enrichAgents(
  $agents: DetectedAgent[],
  $events: FileEvent[],
  $anomaliesByInstance: Record<string, number>,
  $network: NetworkConnection[],
  $fp: FalsePositiveEntry[],
): EnrichedAgent[] {
