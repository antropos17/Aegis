/**
 * @file threat-report.ts — Generate printable HTML threat report
 * @module renderer/utils/threat-report
 * @since 0.2.0
 */

/** Known risk rating levels with associated colors */
type RiskRating = 'CLEAR' | 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL';

/** Analysis result from session or agent threat analysis */
interface ThreatAnalysisResult {
  readonly riskRating: RiskRating | string | null;
  readonly summary: string | null;
  readonly findings?: readonly string[];
  readonly recommendations?: readonly string[];
}

/** Aggregated activity counts for the report header */
...
/** Color map for known risk ratings */
...
/** Fallback color for unknown risk ratings */
const FALLBACK_COLOR = '#ED8936';
 ...
 * @param result - Analysis result (riskRating, summary, findings, recommendations)
 * @param counts - Aggregated activity counts
 */
export function openThreatReport(result: ThreatAnalysisResult, counts: ReportCounts): void {
  const now = new Date().toLocaleString();
  const rating = result.riskRating as RiskRating;
  const rc = RATING_COLORS[rating] || FALLBACK_COLOR;
