...
function classifySensitive(filePath) {
  for (const rule of SENSITIVE_RULES) { if (rule.pattern.test(filePath)) return rule.reason; }
  if (_state) { for (const rule of _state.getCustomRules()) { if (rule.pattern.test(filePath)) return rule.reason; } }
  return null;
}
...
function handleWatcherEvent(action, filePath) {
  ...
  if (watcherDebounce.size > 500) { for (const [k, t] of watcherDebounce) { if (now - t > 10000) watcherDebounce.delete(k); } }
...
function setupFileWatchers() {
  ...
  bindWatcherEvents(ew); _state.watchers.push(ew);
}
...
function scanFileHandles(agent) {
  return new Promise((resolve) => {
    const pid = agent.pid;
    const psScript = ['$ErrorActionPreference="SilentlyContinue"', '$files=[System.Collections.ArrayList]@()', '$h=Get-Command handle64.exe -EA SilentlyContinue', 'if(!$h){$h=Get-Command handle.exe -EA SilentlyContinue}', 'if($h){', `  $out=& $h.Source -p ${pid} -nobanner -accepteula 2>$null`, '  foreach($l in $out){', '    if($l -match "File\\s+.*?\\s+([A-Z]:\\\\.+)$"){', '      [void]$files.Add($Matches[1].Trim())', '    }', '  }', '}else{', `  $p=Get-Process -Id ${pid} -EA SilentlyContinue`, '  if($p -and $p.Modules){', '    foreach($m in $p.Modules){', '      if($m.FileName){[void]$files.Add($m.FileName)}', '    }', '  }', '}', 'if($files.Count -gt 0){$files|ConvertTo-Json -Compress}else{"[]"}'].join('\n');
    execFile('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', psScript], { timeout: 15000 }, (err, stdout) => {
...
async function scanAllFileHandles(agents) {
  const toScan = (_state && _state.isOtherPanelExpanded()) ? agents : agents.filter(a => a.category === 'ai');
  const allNew = [];
  for (const agent of toScan) { const events = await scanFileHandles(agent); allNew.push(...events); }
  return allNew;
}
...
function pruneKnownHandles(activeAgents) {
  ...
  for (const pid of _state.knownHandles.keys()) { if (!activePids.has(pid)) _state.knownHandles.delete(pid); }
}

module.exports = { init, setupFileWatchers, scanAllFileHandles, pruneKnownHandles, classifySensitive, shouldIgnore };
