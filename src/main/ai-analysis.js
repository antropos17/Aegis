...
function analyzeSessionActivity() {
  return new Promise((resolve) => {
    const apiKey = _state.getSettings().anthropicApiKey;
    if (!apiKey) { resolve({ success: false, error: 'Set your Anthropic API key in Settings' }); return; }
    ...
    const configAccessEvents = allEvents.filter(e => e.reason && e.reason.startsWith('AI agent config'));
    ...
    for (const ev of allEvents) {
      if (!agentSummaries[ev.agent]) agentSummaries[ev.agent] = { files: 0, sensitive: 0, configAccess: 0, reasons: new Set() };
      agentSummaries[ev.agent].files++;
      if (ev.sensitive) { agentSummaries[ev.agent].sensitive++; agentSummaries[ev.agent].reasons.add(ev.reason); }
      if (ev.reason && ev.reason.startsWith('AI agent config')) agentSummaries[ev.agent].configAccess++;
    }
    let agentSection = '';
    ...
    const sensitiveDetails = sensitiveEvents.slice(-30).map(e => `  - [${e.agent}] ${e.action}: ${e.file} (${e.reason})`).join('\n') || '  (none)';
    const configDetails = configAccessEvents.slice(-20).map(e => `  - [${e.agent}] ${e.action}: ${e.file} (${e.reason})`).join('\n') || '  (none)';
    const netDetails = netConns.map(c => {
      const flag = c.flagged ? ' [FLAGGED]' : '';
    ...
    const userMessage = [
      ...
      '', 'Per-agent summary:', agentSection,
      'Recent sensitive file accesses (last 30):', sensitiveDetails, '',
      'AI agent config accesses:', configDetails, '',
      'Network connections:', netDetails,
    ].join('\n');
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        } catch (_) { resolve({ success: false, error: 'Failed to parse API response' }); }
      });
    });
    req.on('error', (err) => { resolve({ success: false, error: err.message || 'Network error' }); });
    req.setTimeout(60000, () => { req.destroy(); resolve({ success: false, error: 'Request timed out (60s)' }); });
    req.write(body);
    req.end();
  });
}
