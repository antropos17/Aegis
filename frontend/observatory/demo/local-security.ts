...
export async function previewLocalSecurity(request: unknown): Promise<RecordData> {
  ...
  const local = {
    ...
    reviewRequired: true,
    ruleSet: { id: 'aegis-local-static', version: 9 },
    findings: [
      {
        ruleId: 'STA014',
        severity: 'medium',
  ...
  const inventory = {
    ...
    components: [
      ...files,
      {
        ...
        provenance: { agent: 'shared', scope: 'project', basis: 'selected-layout' },
      },
      {
        ...
        geminiMcpDeclarations: {
          ...
          includeToolsEntries: 2,
          excludeToolsServers: 0,
          excludeToolsEntries: 0,
        },
        provenance: { agent: 'gemini-cli', scope: 'project', basis: 'selected-layout' },
      },
    ],
    issues: [],
    catalog,
