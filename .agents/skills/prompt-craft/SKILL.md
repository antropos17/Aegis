---
name: prompt-craft
description: Aegis prompt formula for Codex and Antigravity. Use when writing prompts, planning tasks, starting new work, or when the user says "write a prompt", "create a task", "plan work", or asks how to structure a request for an AI coding assistant. Also use proactively when decomposing a large request into atomic subtasks.
---

# Prompt Craft — Aegis Task Formula

A prompt in Aegis is not just a request — it is a **complete execution plan** that an AI coding assistant (Codex or Antigravity) can follow without ambiguity. A well-written prompt eliminates back-and-forth, prevents scope creep, and guarantees that every change is verified before it touches the codebase.
...
- **agent-database.json changed:** Verify count consistency across README, AGENTS.md
