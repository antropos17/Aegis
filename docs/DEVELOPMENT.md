...
- **300-line soft limit** per file — a target for new files. Extract when adding to an oversized file. The current 32 existing `src/` files above 300 lines (JSON excluded) are tracked by `npm run counts:check`; this size target is not enforced by the linter
