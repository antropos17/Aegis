...
`--static-scan-json` can associate literal command values and simple wrapper calls
across selected JavaScript or Python files. For example, a caller passing a literal
download-and-execute command to a helper function can now receive STA001, with
evidence for both the caller and helper. The command is never executed.
...
JavaScript supports selected plain function/arrow shapes with positional name
parameters, optional immutable declarations and one final call. Python supports
undecorated module-level synchronous functions with plain positional parameters
...
Reports retain schema version 1 and use `aegis-static-patterns` version 4.
...
- `flow.files`: sorted unique paths and original hashes for contributing callers,
  imported values, re-exports and function bodies.
...
atomic project snapshot or proof of execution. New `codeFlow` scope fields and
flow limits make pre-flow reports incompatible as external-import baselines;
fresh comparison is required. Nothing is automatically trusted or accepted.
...
| Module/import or wrapper depth | 8 |
| Prepared module models per entry file | 32 |
| Wrapper expansions per entry file | 256 |
...
General dataflow through arbitrary objects, function returns, closures, module
side effects and runtime dependencies remains outside this subset. It adds no
...
mutation, cycles, malformed evidence, ambiguous imports, redaction and non-execution.
