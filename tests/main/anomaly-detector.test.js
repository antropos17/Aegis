...
describe('anomaly-detector', () => {
  ...
  beforeEach(() => {
    // createRequire needed here for CJS cache management — anomaly-detector.js
    // internally require()'s baselines.js, so both must share the same CJS cache
  ...
  function setupAgent(agentName, sessionOverrides, baselineOverrides) {
    ...
    if (sessionOverrides.sensitiveCount !== undefined)
      sd.sensitiveCount = sessionOverrides.sensitiveCount;
  ...
  describe('dimensions breakdown', () => {
    ...
    it('process dimension includes new categories and directory factors', () => {
      ...
      expect(dimensions.process.factors.some((f) => f.includes('new sensitive category'))).toBe(
        true,
      );
