...
## Key Paths
- src/main/ — 58 CommonJS modules (46 top-level + platform/ 10 + token-adapters/ 2)
...
- rules/sequences/ — 1 sequence correlation rule (SEQ001) in 1 sequence rule file, read by src/main/sequence-rule-loader.js, consumed by src/main/sequence-engine.js through the five scan taps in main.js and scan-loop.js, hot-reloaded by a second watcher (setupSequenceRulesWatcher in src/main/file-watcher.js), gated by `npm run verify:seq-gate` (docs/roadmap/sequence-rules.md)
