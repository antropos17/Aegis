...
`X:/tmp/aegis-etw-burst-20260911`, ветка `codex/etw-burst-handling`.
...
Это синтетическая проверка изоляции стадий. Новый реальный UAC/ETW-замер не запускали,
12 061 старых потерь не объявлять устранёнными. E3 остаётся открыт; следующий шаг —
контрольный live-замер после согласования. Сон отложен. Установленная программа
не заменялась. Перед продолжением проверить окончательные PR/CI/merge этой ветки.
