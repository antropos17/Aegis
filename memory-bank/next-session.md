...
Current continuation: [credential-bound MCP grants](../docs/MCP-DURABLE-GRANTS.md) add opt-in manifest v4 for HTTP/HTTPS. An explicit CLI prepares a private store key and HMAC tag for the selected endpoint bearer. The gateway requires that key and compares tag plus v3 public route before network connection or grant consumption. The manifest contains neither token nor a simple token hash. This binds selected bytes, not verified server/account identity; v1–v3, stdio and outside-route limits remain. Check the implementation PR and all five CI contexts before claiming publication.
