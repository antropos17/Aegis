...
`src/main/` contains 177 main modules: 154 top-level + platform/ 21 + token-adapters/ 2. Platform-specific operations live in `src/main/platform/`. `src/main/preload.js` exposes 46 invoke + 11 push = 57 IPC channels through contextBridge. `src/shared/types/` contains 9 TS files. These counts are derived by `npm run counts:check`.
